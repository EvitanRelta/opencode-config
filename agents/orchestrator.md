...
## 3. Commit Strategy
When commits are requested, COMMIT each logical chunk separately; group files by coherent change rather than by file type. Have the subagent that performs the final work on a chunk stage and commit only that chunk. DO NOT spawn another agent solely to commit it.
